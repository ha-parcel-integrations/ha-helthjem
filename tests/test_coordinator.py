"""Tests for the Helthjem coordinator: fetching, caching, polling and events.
...
from custom_components.helthjem.api import HelthjemApiError
...
from custom_components.helthjem.coordinator import (
    HelthjemCoordinator,
...
async def test_polling_stops_when_everything_delivered(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = delivered_sample(DELIVERED_CODE)
    coordinator = HelthjemCoordinator(hass, client, entry)
...
async def test_polling_is_hot_for_an_out_for_delivery_parcel(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = active_sample(ACTIVE_CODE)  # OUT_FOR_DELIVERY
    coordinator = HelthjemCoordinator(hass, client, entry)
...
async def test_polling_is_mid_for_an_in_transit_parcel(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = _in_transit(ACTIVE_CODE)
    coordinator = HelthjemCoordinator(hass, client, entry)
...
async def test_polling_resumes_when_a_parcel_is_tracked_again(hass):
    """Adding a parcel back after a full stop recomputes the tier and re-arms."""
    entry = _entry_with([])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = HelthjemCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = HelthjemApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    moved["estimatedDelivery"] = {"date": "2026-04-29T16:00:00Z"}
...
async def test_losing_the_eta_is_silent(hass):
    ...
    dropped["estimatedDelivery"] = {"date": None}
