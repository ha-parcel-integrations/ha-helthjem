...
def test_build_history_orders_oldest_to_newest():
    ...
    assert history[0]["raw_status"] == "Sendingen er registrert"
...
def test_normalize_delivered_parcel():
    ...
    assert parcel["barcode"] == "TESTHJEM00000002"
    # Helthjem's tracking type exposes neither sender nor receiver name.
    assert parcel["sender"] is None
    assert parcel["receiver"] is None
    assert parcel["status"] == ParcelStatus.DELIVERED
    # raw_status is Helthjem's own status enum value.
    assert parcel["raw_status"] == "DELIVERED"
    assert parcel["delivered"] is True
    # No top-level delivered timestamp — taken from the DELIVERED event.
    assert parcel["delivered_at"] == "2026-04-29T13:12:42Z"
    # A delivered parcel drops its ETA — meaningless once it has arrived.
    assert parcel["planned_from"] is None
    assert parcel["planned_to"] is None
    assert parcel["url"] == (
        "https://helthjem.no/sporing/?trackingReference=TESTHJEM00000002"
    )
    # Not exposed by Helthjem's tracking type.
...
def test_normalize_active_parcel_has_point_eta():
    """Helthjem reports a single estimated date, never a window."""
    ...
    assert parcel["planned_to"] is None  # never a window
...
def test_normalize_pickup_parcel():
    ...
    assert parcel["pickup_point"] == "Helthjem Hentepunkt Grünerløkka"


def test_normalize_warns_once_on_unconfirmed_delivery_point_type(caplog):
    """Pre-release: an observed DeliveryPointType is logged once for us to map."""
    raw = active_sample()
    raw["deliveryPoint"] = {"type": "SOME_UNSEEN_POINT_TYPE"}
    normalize_parcel(raw)
    normalize_parcel(raw)
    assert caplog.text.count("SOME_UNSEEN_POINT_TYPE") == 1
    assert "issues/new" in caplog.text
...
def test_normalize_never_exposes_sender_or_receiver():
    """Helthjem's tracking type carries no personal names — always None."""
    parcel = normalize_parcel(active_sample())
    assert parcel["sender"] is None
    assert parcel["receiver"] is None
...
def test_normalize_raw_status_is_the_status_enum():
    raw = active_sample()
