"""Sensor platform for the Helthjem parcel tracker integration."""
...
from .coordinator import HelthjemCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: HelthjemConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Helthjem sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        HelthjemIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        HelthjemAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            HelthjemParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(HelthjemNextDeliverySensor(coordinator, entry))
    entities.append(HelthjemDeliveredParcelsSensor(coordinator, entry))
    entities.append(HelthjemLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class HelthjemAwaitingPickupSensor(
    CoordinatorEntity[HelthjemCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: HelthjemCoordinator, entry: ConfigEntry) -> None:
...
class HelthjemIncomingParcelsSensor(
    CoordinatorEntity[HelthjemCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                HelthjemParcelSensor(self.coordinator, self._entry, barcode)
...
class HelthjemParcelSensor(CoordinatorEntity[HelthjemCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Helthjem parcel."""
    ...
    def __init__(
        self, coordinator: HelthjemCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class HelthjemNextDeliverySensor(
    CoordinatorEntity[HelthjemCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: HelthjemCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class HelthjemDeliveredParcelsSensor(
    CoordinatorEntity[HelthjemCoordinator], SensorEntity
):
    """Recently delivered tracked Helthjem parcels."""
...
class HelthjemLastUpdateSensor(
    CoordinatorEntity[HelthjemCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Helthjem was last polled successfully."""
