"""Config flow for the Helthjem parcel tracker integration."""
...
class HelthjemConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Helthjem integration."""
    ...
    ) -> HelthjemOptionsFlowHandler:
        """Return the options flow handler."""
        return HelthjemOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the Helthjem hub — single instance, no input needed.
        ...
        the ``helthjem.track_parcel`` service or a dashboard button.
        ``single_config_entry`` in the manifest enforces one hub. Helthjem's
        GraphQL endpoint keys on the parcel reference alone — no postcode — so
        there is genuinely nothing to ask here.
        """
...
class HelthjemOptionsFlowHandler(OptionsFlow):
