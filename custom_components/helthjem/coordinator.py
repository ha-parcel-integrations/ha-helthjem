"""Coordinator for the Helthjem parcel tracker integration.
...
from .api import HelthjemApiClient, HelthjemApiError
from .const import (
...
class HelthjemCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (HelthjemApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Helthjem fetch failed for %s: %s", code, result)
        ...
        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Helthjem unreachable for all tracked parcels")
